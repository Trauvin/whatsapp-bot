...
def bot():
    incoming_msg = request.values.get('Body', '').lower() # pega a message enviada pelo usuário
    resp = MessagingResponse()
    msg = resp.message()
    responded = False

    if 'nun' in incoming_msg:
        # retorne uma citação 
        msg.body("A linha da pobreza no Brasil.")
        responded = True
    
    if 'brasil' and 'descoberto' in incoming_msg:
        msg.body("No dia 26 de Janeiro de 1500, por Pedro Álvares Cabral.")
        responded = True
